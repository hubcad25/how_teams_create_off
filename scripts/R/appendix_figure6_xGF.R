# APPENDIX: xGF/60 by cluster
# Strip plot showing expected offensive production by cluster
...
# Join data
df_output <- df_teams %>%
  left_join(df_metrics %>% select(team, input_xGoals_per60), by = "team")

league_avg <- mean(df_output$input_xGoals_per60)

# Order clusters by mean xGF/60
cluster_means <- df_output %>%
  group_by(cluster) %>%
  summarise(
    mean_goals = mean(input_xGoals_per60),
    sd_goals = sd(input_xGoals_per60),
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(
...
df_strip <- df_output %>%
  arrange(cluster_ordered, input_xGoals_per60) %>%
...
# CREATE PLOT
cat("Creating xGF/60 strip plot...\n")

p_strip <- ggplot(df_strip, aes(x = input_xGoals_per60, y = y_dodge, color = cluster)) +
  ...
  labs(
    title = "xGF/60 by Offensive Archetype",
...
# Save
ggsave("outputs/figures/appendix_figure6_xGF.png", p_strip,
       width = 11, height = 8, dpi = 300)

cat("Saved: outputs/figures/appendix_figure6_xGF.png\n")
print(p_strip)
